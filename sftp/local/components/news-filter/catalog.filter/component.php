		...
		case "L":
			...
			if ('C' == $arProp['LIST_TYPE'])
			{
				...
				else
				{
					...
					$arListRadio[] = '<div class="filter__news__radio"><input hidden id="news-item-10" id="'.$val.'" type="radio" name="'.$name.'" value=""'.($key == $value ? ' checked' : '').'>'.'<label for="news-item-10">'.GetMessage("CC_BCF_ALL").'</label>'.'</div>';
					foreach ($arProp["VALUE_LIST"] as $key=>$val)
					{
						$arListRadio[] = '<div class="filter__news__radio"><input hidden id="news-item-'.htmlspecialcharsbx($key).'" type="radio" name="'.$name.'" value="'.htmlspecialcharsbx($key).'"'.($key == $value ? ' checked' : '').'>'.'<label for="news-item-'.htmlspecialcharsbx($key).'">'.htmlspecialcharsex($val).'</label>'.'</div>';
						$list[$key] = $val;
					}
				}
